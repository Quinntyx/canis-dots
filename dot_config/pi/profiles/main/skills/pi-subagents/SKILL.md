...
- **Interrupts never kill the session.** Aborting the call (Esc) or hitting the idle timeout sends Ctrl-C into the interpreter: the chunk stops where it is, and you get back the exact line plus the Python traceback (`Stopped at: chunk line 4: await subagents.wait_all_async(handles)`). The namespace survives, so `handles` and friends are still usable — an interrupted fan-out can simply be awaited again in a later chunk: `responses = await subagents.wait_all_async(handles)`. Use `/ptc interrupt` to stop a running chunk on purpose, `/ptc kill` to drop the session.
